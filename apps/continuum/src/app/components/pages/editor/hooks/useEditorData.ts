...
export function useEditorData() {
  ...
  const activePosts = rpc.post.getAllAdminPosts.useQuery(undefined, {
  ...
  const trashedPosts = rpc.post.getTrashedPosts.useQuery(undefined, {
  ...
  const specificPost = rpc.post.getDetailedPublicPost.useQuery(
    { slug: blogSlug ?? "" },
    {
      enabled:
        isLoggedIn &&
